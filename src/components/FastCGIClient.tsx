...
export default function FastCGIClient({ onBack }: FastCGIProps) {
  ...
  const [host, setHost] = usePersistedState('fastcgi-host', '');
  const [port, setPort] = usePersistedState('fastcgi-port', '9000');
  const [activeTab, setActiveTab] = useState<TabType>('probe');
  const [loading, setLoading] = useState(false);

  // Request-specific fields
  const [scriptFilename, setScriptFilename] = usePersistedState('fastcgi-scriptFilename', '/index.php');
  const [requestUri, setRequestUri] = usePersistedState('fastcgi-requestUri', '/');
    ...
    <div className="max-w-4xl mx-auto">      {/* Header */}
      <div className="flex items-center gap-4 mb-8">
        ...
        <div className="flex items-center gap-3">
          ...
          <div>
            <h1 className={`text-2xl font-bold $text-white`}>FastCGI Client</h1>
            ...
            <>
              ...
              <div className="grid grid-cols-2 md:grid-cols-4 gap-4 mb-6">
                <div className={`$bg-slate-900/50 border border-slate-700 rounded-lg p-3 text-center`}>
                  <p className={`text-xs $text-slate-400`}>Max Conns</p>
                  <p className={`text-lg font-bold $text-white`}>
                    {probeResult.maxConns ?? 'N/A'}
                  </p>
                </div>
                <div className={`$bg-slate-900/50 border border-slate-700 rounded-lg p-3 text-center`}>
                  <p className={`text-xs $text-slate-400`}>Max Requests</p>
                  <p className={`text-lg font-bold $text-white`}>
                    {probeResult.maxReqs ?? 'N/A'}
                  </p>
                </div>
                <div className={`$bg-slate-900/50 border border-slate-700 rounded-lg p-3 text-center`}>
                  <p className={`text-xs $text-slate-400`}>Multiplexing</p>
                  <p className={`text-lg font-bold ${probeResult.multiplexing ? 'text-green-400' : 'text-slate-400'}`}>
                      ...
                      <tbody>
                        {Object.entries(probeResult.serverValues).map(([key, value]) => (
                          <tr key={key} className="border-b border-slate-700">
                            ...
                            <td className={`px-4 py-2 font-mono $text-white`}>{value}</td>
                          </tr>
                        ))}
                      </tbody>
                    </table>
                ...
                <div>
                  ...
                  <div className="space-y-2">
                    {probeResult.records.map((record, i) => (
                      <div key={i} className={`p-2 rounded-lg $bg-slate-900/50 border border-slate-700`}>
                    ...
                    <table className="w-full text-sm">
                      <tbody>
                        {Object.entries(requestResult.headers).map(([key, value]) => (
                          <tr key={key} className="border-b border-slate-700">
                            <td className={`px-4 py-2 font-medium $text-slate-400`}>{key}</td>
                  ...
                  <div className="flex flex-wrap gap-2">
                    {requestResult.records.map((record, i) => (
                      <span key={i} className={`px-2 py-1 rounded text-xs font-mono ${
                        'bg-slate-900/50 border border-slate-700 text-orange-400'
                      }`}>
      ...
      <div className={`$bg-slate-800 border border-slate-700 rounded-xl p-6`}>
        ...
        <div className={`space-y-3 text-sm $text-slate-300`}>
          <p>
            FastCGI is a <strong className="text-white">binary protocol</strong> for
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 gap-4 mt-3">
            ...
            <div>
              <h3 className={`font-medium mb-2 $text-white`}>Common Stacks</h3>
              <ul className="list-disc list-inside space-y-1">
                <li>Nginx + PHP-FPM</li>
