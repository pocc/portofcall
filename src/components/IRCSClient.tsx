...
export default function IRCSClient({ onBack }: IRCSClientProps) {
  const [host, setHost] = usePersistedState('ircs-host', 'irc.libera.chat');
  const [port, setPort] = usePersistedState('ircs-port', '6697');
  const [nickname, setNickname] = usePersistedState('ircs-nickname', '');
  const [password, setPassword] = useState('');
  const [autoJoinChannels, setAutoJoinChannels] = usePersistedState('ircs-autoJoinChannels', '#portofcall');
  ...
  const handleConnect = () => {
    ...
    addServerMessage(`Connecting to ${host}:${port} (TLS)...`, 'info');
    ...
    const wsUrl = `${protocol}//${window.location.host}/api/ircs/connect?${params}`;
    ...
    <div className="max-w-7xl mx-auto">
      ...
      <div className="mb-6 flex items-center justify-between">
        <div className="flex items-center gap-4">
          ...
          <h1 className="text-3xl font-bold text-white">IRCS Client</h1>
          <span className="text-xs bg-green-600/30 text-green-400 px-2 py-0.5 rounded-full">TLS</span>
        </div>
        {connected && (
          <div className="flex items-center gap-2">
            <div className="w-2 h-2 bg-green-400 rounded-full animate-pulse"></div>
            <span className="text-green-400 text-sm">
              {currentNick}@{host}:{port} (TLS)
            </span>
          </div>
        )}
      </div>
      ...
      {!connected ? (
        ...
        <div className="max-w-lg mx-auto">
          <div className="bg-slate-800 border border-slate-600 rounded-xl p-6">
            <h2 className="text-xl font-semibold text-white mb-4">Connect to IRC Server (TLS)</h2>
            <div className="space-y-4">
              ...
              >
                {loading ? 'Connecting...' : 'Connect (TLS)'}
              </button>
            </div>

            <div className="mt-6 pt-6 border-t border-slate-600">
              <h3 className="text-sm font-semibold text-slate-300 mb-2">About IRCS</h3>
              <p className="text-xs text-slate-400">
                IRCS (IRC over TLS, RFC 7194) provides encrypted IRC connections on port 6697.
                Unlike STARTTLS which upgrades a plain connection, IRCS uses implicit TLS from
                the start. Most modern IRC networks support TLS: irc.libera.chat:6697,
                irc.oftc.net:6697, irc.efnet.org:6697.
              </p>
          ...
          <div className="col-span-4 bg-slate-800 border border-slate-600 rounded-xl flex flex-col">
            <div className="px-4 py-2 border-b border-slate-600 flex items-center justify-between">
              <div className="flex items-center gap-2">
                ...
                <span className="text-xs bg-green-600/30 text-green-400 px-1.5 py-0.5 rounded">TLS</span>
              </div>
