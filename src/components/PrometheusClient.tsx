...
export default function PrometheusClient({ onBack }: PrometheusProps) {
  ...
  const [host, setHost] = usePersistedState('prometheus-host', '');
  const [port, setPort] = usePersistedState('prometheus-port', '9090');
  const [query, setQuery] = usePersistedState('prometheus-query', 'up');
    ...
    <div className="max-w-4xl mx-auto">
      ...
      <div className="flex items-center gap-4 mb-8">
        ...
        <div className="flex items-center gap-3">
          ...
          <div>
            <h1 className={`text-2xl font-bold $text-white`}>Prometheus Client</h1>
                  ...
                  <tbody>
                    {healthResult.version && (
                      <tr className="border-b border-slate-700">
                        <td className={`px-4 py-2 font-medium $text-slate-400`}>Version</td>
                        <td className={`px-4 py-2 $text-white`}>{healthResult.version}</td>
                      </tr>
                    )}
                    {healthResult.revision && (
                      <tr className="border-b border-slate-700">
                        <td className={`px-4 py-2 font-medium $text-slate-400`}>Revision</td>
                        <td className={`px-4 py-2 font-mono $text-white`}>{healthResult.revision}</td>
                      </tr>
                    )}
                    {healthResult.goVersion && (
                      <tr className="border-b border-slate-700">
                        <td className={`px-4 py-2 font-medium $text-slate-400`}>Go Version</td>
                        <td className={`px-4 py-2 $text-white`}>{healthResult.goVersion}</td>
                      </tr>
                    )}
                    {healthResult.branch && (
                      <tr className="border-b border-slate-700">
                        <td className={`px-4 py-2 font-medium $text-slate-400`}>Branch</td>
                        <td className={`px-4 py-2 $text-white`}>{healthResult.branch}</td>
                      </tr>
                    )}
                    {healthResult.activeTargets !== null && healthResult.activeTargets !== undefined && (
                      <tr className="border-b border-slate-700">
                        <td className={`px-4 py-2 font-medium $text-slate-400`}>Active Targets</td>
                        <td className={`px-4 py-2 $text-white`}>{healthResult.activeTargets}</td>
                      </tr>
                    )}
                  </tbody>
                </table>
          ...
          {!queryResult.success ? (
            ...
            <>
              ...
              <div className={`mb-4 p-3 rounded-lg $bg-slate-900/50 border border-slate-700`}>
                ...
                <p className={`font-mono $text-red-300`}>{queryResult.query}</p>
              </div>
              ...
              {queryResult.results && queryResult.results.length > 0 && (
                <div className={`$bg-slate-900/50 border border-slate-700 rounded-lg overflow-hidden`}>
                  <table className="w-full text-sm">
                    <thead>
                      <tr className="border-b border-slate-700">
                        <th className={`px-4 py-2 text-left $text-slate-400`}>Metric</th>
                        <th className={`px-4 py-2 text-right $text-slate-400`}>Value</th>
                      </tr>
                    </thead>
                    <tbody>
                        ...
                        <tr key={i} className="border-b border-slate-700">
                          <td className={`px-4 py-2 $text-white`}>
                            <span className="font-mono text-xs">
                              <span className="text-red-300">{r.metric.__name__ || 'unnamed'}</span>
                              <span className="text-slate-400">{formatMetricLabels(r.metric)}</span>
                            </span>
                          </td>
        ...
        <div className={`$bg-slate-800 border border-slate-700 rounded-xl p-6 mb-6`}>
          ...
          {!metricsResult.success ? (
            ...
            <>
              <div className="grid grid-cols-2 md:grid-cols-4 gap-4 mb-6">
                <div className={`$bg-slate-900/50 border border-slate-700 rounded-lg p-3 text-center`}>
                  <p className={`text-xs $text-slate-400`}>Metric Families</p>
                  <p className={`text-lg font-bold $text-white`}>
                  ...
                  <div className="space-y-1">
                    {metricsResult.preview.map((m, i) => (
                      <div key={i} className="flex justify-between gap-4 font-mono text-xs">
                        ...
                        <span className={`$text-green-400 whitespace-nowrap`}>{m.value}</span>
                      </div>
                    ))}
                  </div>
                </div>
      ...
      <div className={`$bg-slate-800 border border-slate-700 rounded-xl p-6`}>
        ...
        <div className={`space-y-3 text-sm $text-slate-300`}>
          <p>
            ...
            <strong className="text-white"> monitoring and alerting toolkit</strong> for
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 gap-4 mt-3">
            ...
            <div>
              <h3 className={`font-medium mb-2 $text-white`}>Common Queries</h3>
              <ul className="list-disc list-inside space-y-1">
