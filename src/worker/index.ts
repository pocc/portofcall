...
export default {
  async fetch(request: Request, env: Env, _ctx: ExecutionContext): Promise<Response> {
    ...
    // Applies to both WebSocket upgrades and HTTP API requests.
    ...
    if (isWebSocketUpgrade) {
      return executeRequest().then(r => addSecurityHeaders(request, r));
    }

    return withRequestTimeoutCleanup(executeRequest)
      .then(r => sanitizeErrors(request, r))
      .then(r => addSecurityHeaders(request, r));
  },
};
