...
export default {
  async fetch(request: Request, env: Env, _ctx: ExecutionContext): Promise<Response> {
    ...
    // --- WebSocket Origin validation (prevent cross-origin hijacking) ---
    ...
    const sanitizeErrors = async (response: Response): Promise<Response> => {
    ...
    if (isWebSocketUpgrade) {
      return executeRequest().then(addSecurityHeaders);
    }

    return withRequestTimeoutCleanup(executeRequest).then(sanitizeErrors).then(addSecurityHeaders);
  },
};
