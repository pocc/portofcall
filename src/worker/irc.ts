...
export async function handleIRCWebSocket(request: Request): Promise<Response> {
  try {
    ...
    // Connect to IRC server in background after receiving credentials
    (async () => {
      try {
        ...
        server.addEventListener('message', async (event) => {
          try {
            ...
            const s = (v: unknown): string => (typeof v === 'string' ? v.replace(/[\r\n]/g, '') : String(v ?? ''));
              ...
              switch (cmd.type) {
                case 'raw':
                  // Send raw IRC command (sanitized to single line)
              ...
              if (!registered && (msg.command === '376' || msg.command === '422')) {
                ...
                if (channels.length > 0) {
                  ...
                  for (const channel of channels) {
                    await joinWriter.write(encoder.encode(`JOIN ${channel.replace(/[\r\n]/g, '')}\r\n`));
                  }
