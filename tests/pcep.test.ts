/**
 * PCEP (Path Computation Element Protocol) Integration Tests
 * Tests OPEN handshake and server detection
 */
...
describe('PCEP Protocol Integration Tests', () => {
  describe('PCEP Connect (HTTP)', () => {
    it('should handle connection to non-existent host', async () => {
      const response = await fetch(`${API_BASE}/pcep/connect`, {
        method: 'POST',
        headers: { 'Content-Type': 'application/json' },
        body: JSON.stringify({
          host: 'non-existent-pce-12345.example.com',
          port: 4189,
          timeout: 5000,
        }),
      });
  ...
  describe('PCEP Probe (HTTP)', () => {
    it('should handle probe to non-existent host', async () => {
      const response = await fetch(`${API_BASE}/pcep/probe`, {
        method: 'POST',
        headers: { 'Content-Type': 'application/json' },
        body: JSON.stringify({
          host: 'non-existent-pce-12345.example.com',
  ...
  describe('PCEP Cloudflare Detection', () => {
